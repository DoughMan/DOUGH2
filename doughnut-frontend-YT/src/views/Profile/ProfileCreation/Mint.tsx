...
import { useDough, useBunnyFactory } from 'hooks/useContract'
...
const minimumDoughBalanceToMint = new BigNumber(MINT_COST).multipliedBy(DEFAULT_TOKEN_DECIMAL)

const Mint: React.FC = () => {
  const [variationId, setVariationId] = useState<Nft['variationId']>(null)
  const { actions, minimumDoughRequired, allowance } = useProfileCreation()

  const { account } = useWeb3React()
  const cakeContract = useDough()
  const bunnyFactoryContract = useBunnyFactory()
  const { t } = useTranslation()
  const hasMinimumDoughRequired = useHasDoughBalance(minimumDoughBalanceToMint)
  ...
  return (
    <>
      ...
      <Text as="p" mb="24px">
        {t('You can change your profile pic later if you get another approved Doughnut Collectible.')}
      </Text>
      <Card mb="24px">
        <CardBody>
          ...
          <Text as="p" mb="24px" color="textSubtle">
            {t('Cost: %num% DOUGH', { num: MINT_COST })}
          </Text>
          {nfts.map((nft) => {
            ...
            return (
              <SelectionCard
                ...
                disabled={isApproving || isConfirming || isConfirmed || !hasMinimumDoughRequired}
              >
          ...
          {!hasMinimumDoughRequired && (
            <Text color="failure" mb="16px">
              {t('A minimum of %num% DOUGH is required', { num: MINT_COST })}
            </Text>
          )}
          <ApproveConfirmButtons
            isApproveDisabled={variationId === null || isConfirmed || isConfirming || isApproved}
            isApproving={isApproving}
            isConfirmDisabled={!isApproved || isConfirmed || !hasMinimumDoughRequired}
