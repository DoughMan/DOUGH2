...
export const useCompetitionDoughRewards = (userDoughReward: ReactText) => {
  const cakeAsBigNumber = new BigNumber(userDoughReward as string)
  const cakeBalance = getBalanceNumber(cakeAsBigNumber)
  const cakePriceBusd = usePriceDoughBusd()
  return {
    cakeReward: cakeBalance,
    dollarValueOfDoughReward: cakePriceBusd.gt(0) ? cakeBalance * cakePriceBusd.toNumber() : null,
  }
}
