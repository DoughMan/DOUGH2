# DoughnutSwap Token Lists

This repo contains main DoughnutSwap token list and tools to validate it.
...
- Add `checksum:newlistname`, `generate:newlistname`, `makelist:newlistname` command to `package.json` analogous to DoughnutSwap default and extended list scripts.
- Modify `checksum.ts`, `buildList.ts` and `default.test.ts` to handle new list

## How to add new tokens to DoughnutSwap (extended) token list
...
```shell script
# Fetch the Top100 Tokens on DoughnutSwap v2, and update list.
...
Netlify simply takes the json files under `lists` directory and hosts them on `tokens.doughnutswap.shop/list-name.json`
